...
using AnnotationsFrameworkSample.Entities;
using System;

[assembly: LambdaSerializer(typeof(DefaultLambdaJsonSerializer))]

namespace AnnotationsFrameworkSample
{
    /// <summary>
    ...
    public class Functions
    {
        ...
        /// </remarks>
        public Functions(ICalculatorService calculatorService, IDynamoDBContext dynamoDBContext)
        {
...
";
        ...
        [LambdaFunction(ResourceName = "GetInstructor")]
        [HttpApi(LambdaHttpMethod.Get, "/instructor/{key}")]
        public async Task<Instructor> GetInstructor(string key, ILambdaContext context)
        {
            string instructorKey = $"instructor#{key}";

            context.Logger.LogInformation($"instructur get called");

            var instructor = await _dynamoDBContext.LoadAsync<Instructor>(instructorKey, instructorKey);
